...
const log = require('../../helpers/winston')
  .getLogger({ name: 'auth-business.ctrl' });
/**
...
const sendResetPasswordTokenUser = async (req, res, next) => {
  try {
    const businessUser = await BusinessUser.getByEmail(req.body.email);
    const token = await businessUser.genJWTPasswordResetToken();
    mailer.sendLetter(
      businessUser.email,
      utils.emailsLettersBuilder.getResetPasswordLetterForBusinessUser(businessUser, {
        token
      }),
      'Follow link to reset password'
    );
    log.info('user %s initialized password reset', businessUser.id);
    return res.json({
      status: 'OK',
